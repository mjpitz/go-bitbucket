// Code generated by go-swagger; DO NOT EDIT.

package users

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"net/http"
	"time"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	cr "github.com/go-openapi/runtime/client"
	"github.com/go-openapi/strfmt"
)

// NewGetUserEmailsEmailParams creates a new GetUserEmailsEmailParams object
// with the default values initialized.
func NewGetUserEmailsEmailParams() *GetUserEmailsEmailParams {
	var ()
	return &GetUserEmailsEmailParams{

		timeout: cr.DefaultTimeout,
	}
}

// NewGetUserEmailsEmailParamsWithTimeout creates a new GetUserEmailsEmailParams object
// with the default values initialized, and the ability to set a timeout on a request
func NewGetUserEmailsEmailParamsWithTimeout(timeout time.Duration) *GetUserEmailsEmailParams {
	var ()
	return &GetUserEmailsEmailParams{

		timeout: timeout,
	}
}

// NewGetUserEmailsEmailParamsWithContext creates a new GetUserEmailsEmailParams object
// with the default values initialized, and the ability to set a context for a request
func NewGetUserEmailsEmailParamsWithContext(ctx context.Context) *GetUserEmailsEmailParams {
	var ()
	return &GetUserEmailsEmailParams{

		Context: ctx,
	}
}

// NewGetUserEmailsEmailParamsWithHTTPClient creates a new GetUserEmailsEmailParams object
// with the default values initialized, and the ability to set a custom HTTPClient for a request
func NewGetUserEmailsEmailParamsWithHTTPClient(client *http.Client) *GetUserEmailsEmailParams {
	var ()
	return &GetUserEmailsEmailParams{
		HTTPClient: client,
	}
}

/*GetUserEmailsEmailParams contains all the parameters to send to the API endpoint
for the get user emails email operation typically these are written to a http.Request
*/
type GetUserEmailsEmailParams struct {

	/*Email*/
	Email string

	timeout    time.Duration
	Context    context.Context
	HTTPClient *http.Client
}

// WithTimeout adds the timeout to the get user emails email params
func (o *GetUserEmailsEmailParams) WithTimeout(timeout time.Duration) *GetUserEmailsEmailParams {
	o.SetTimeout(timeout)
	return o
}

// SetTimeout adds the timeout to the get user emails email params
func (o *GetUserEmailsEmailParams) SetTimeout(timeout time.Duration) {
	o.timeout = timeout
}

// WithContext adds the context to the get user emails email params
func (o *GetUserEmailsEmailParams) WithContext(ctx context.Context) *GetUserEmailsEmailParams {
	o.SetContext(ctx)
	return o
}

// SetContext adds the context to the get user emails email params
func (o *GetUserEmailsEmailParams) SetContext(ctx context.Context) {
	o.Context = ctx
}

// WithHTTPClient adds the HTTPClient to the get user emails email params
func (o *GetUserEmailsEmailParams) WithHTTPClient(client *http.Client) *GetUserEmailsEmailParams {
	o.SetHTTPClient(client)
	return o
}

// SetHTTPClient adds the HTTPClient to the get user emails email params
func (o *GetUserEmailsEmailParams) SetHTTPClient(client *http.Client) {
	o.HTTPClient = client
}

// WithEmail adds the email to the get user emails email params
func (o *GetUserEmailsEmailParams) WithEmail(email string) *GetUserEmailsEmailParams {
	o.SetEmail(email)
	return o
}

// SetEmail adds the email to the get user emails email params
func (o *GetUserEmailsEmailParams) SetEmail(email string) {
	o.Email = email
}

// WriteToRequest writes these params to a swagger request
func (o *GetUserEmailsEmailParams) WriteToRequest(r runtime.ClientRequest, reg strfmt.Registry) error {

	if err := r.SetTimeout(o.timeout); err != nil {
		return err
	}
	var res []error

	// path param email
	if err := r.SetPathParam("email", o.Email); err != nil {
		return err
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}
